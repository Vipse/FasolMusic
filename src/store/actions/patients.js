...
export const setFreeIntervals = (freeIntervals, discipline ) => {
    ...
    return (dispatch, getState) => {
       let obj = { discipline }
        axios.post('/catalog.fasol/getMasterList', JSON.stringify(obj))
            .then((rez) => {
                rez.data.result.masterlist[0].idMaster = 3403;
                rez.data.result.masterlist[1].idMaster = 3405;
                rez.data.result.masterlist[2].idMaster = 3420;
                dispatch({
